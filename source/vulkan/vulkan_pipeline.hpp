...
namespace vk {
	...
	class graphics_pipeline : public i_device_object, public i_pipeline {
		...
		graphics_pipeline(vk::device& t_device,  vk::render_pass& t_render_pass, vk::pipeline_layout& t_pipeline_layout
			, VkExtent2D t_extent, VkFormat t_format
			, const std::vector<vk::shader_module*>& t_shader_modules, const vk::vertex_definition& t_definition);
		~graphics_pipeline();
